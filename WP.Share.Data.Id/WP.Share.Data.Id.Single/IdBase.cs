...
namespace WP.Share.Data.Id.Single
{
    internal sealed class IdBase
    {
        ...
        /// sequence支持最大的ID
        /// </summary>
        private static long sequencemax = (1L << IdSetting.SEQUENCE_SIZE) - 1;
        ...
        /// <returns></returns>
        public static long GetId(DateTime createTime, SectionIdType type)
        {
            long timeSpan = (long)(createTime - IdSetting.RelativeTime).TotalSeconds;

            long id = signalID
                   | (timeSpan << deltasecondleft) >> IdSetting.SIGNAL_SIZE 
                    | (worknodeid);

            long sequenceCount = 0L;
            switch (type)
            {
                case SectionIdType.First:
                    break;
                case SectionIdType.Last:
                    sequenceCount = sequencemax;
                    break;
                case SectionIdType.Random:
                    sequenceCount = sequencemax;
                    sequenceCount = new Random().Next(0, (int)sequencemax);
                    break;
            }

            return id | sequenceCount;
        }
